...
import GameAPI, { IGameData, IGameFilter } from "../../api/Games.api";
...
const Games = (): JSX.Element => {
    ...
    useEffect(() => {
        setPage(1)
        findGames()
    }, [page, findName, player, time, hardless])
    const getGames = () => {
        return (
                    ...
                    return (
                        <div className={styles.Line} key={"line" + num}>
                            {Array.from(Array(4).keys()).map(ind => {
                                return <Link to={'/game/' + games[num * 4 + ind].id} className={styles.Item} key={num * 4 + ind}><GameCard game={games[num * 4 + ind]} /></Link>
                            })}
                        </div>
                    )
                })}
    ...
    const getPageRouter = () => {
        const masPages = [page];
        for (let a = page - 1; a > 0 && a >= page - 5; a--)
            masPages.unshift(a)
        ...
        return (
            <div className={styles.PageRouter}>
                <BackPage className={styles.Svg} onClick={() => { toPage(page - 1) }} />
                {
                    masPages.map(a => {
                        if (a == page)
                            return <div key={'page' + a} className={styles.ThisPage} onClick={() => { toPage(a) }} >{a}</div>
                        return <div key={'page' + a} onClick={() => { toPage(a) }} >{a}</div>
                    })
                }
                <BackPage className={styles.Next + ' ' + styles.Svg} onClick={() => { toPage(page + 1) }} />
            </div>
        );
    }
    const getMain = () => {
