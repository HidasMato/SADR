...
class UserService {
    ...
    async getMasterActive({
        id,
    }: {
        id: number;
    }): Promise<boolean | undefined> {
        return (
            await pool.query(`SELECT active FROM masters WHERE id = $1`, [id])
        )?.rows?.[0]?.active;
    }
    ...
    async getUserList({ setting, filter, MODE }: UserSetting) {
        ...
        return (
            await pool.query(`SELECT * FROM users LIMIT $1 OFFSET $2;`, [
                setting.count,
                setting.start,
            ])
        ).rows as UserToCookie[];
    }
    async changePass({
        id,
        cache,
    ...
    async getMyLinkActivate({
        link,
    }: {
        link: string;
    }): Promise<{ userid: number; dateend: Date; mail: string } | undefined> {
        return (
            await pool.query(
    ...
    async deleteMyLink({ userid }: { userid: number }): Promise<boolean> {
        return (
            ((
                await pool.query(`DELETE FROM maillink WHERE userid = $1;`, [
                    userid,
                ])
            ).rowCount as number) > 0
        );
    }
    async changeMailVerifity({
        userid,
        mail,
        value,
    }: {
    ...
    async findUserMail({
        ...
        undefined | { passcache: string; id: number; nickname: string }
    > {
        return (
            await pool.query(
