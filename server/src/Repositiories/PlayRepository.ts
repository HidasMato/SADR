...
class PlayRepository {
    async getPlayInfoById({ id }: { id: number }): Promise<PlayQuery> {
        return (await pool.query(`SELECT * FROM plays WHERE id = $1`, [id]))
            ?.rows?.[0];
    }
    async getPlayList({ setting, filter }: PlaySetting): Promise<PlayQuery[]> {
        ...
        return (
            await pool.query(`SELECT * FROM plays LIMIT $1 OFFSET $2;`, [
                setting.count,
                setting.start,
            ])
        ).rows as PlayQuery[];
    ...
    async deletePlay({ id }: { id: number }): Promise<boolean> {
        return ((await pool.query(`DELETE FROM plays WHERE id = $1;`, [id]))
    ...
    async getAllMastersPlays({
        masterId,
    }: {
        masterId: number;
    }): Promise<number[]> {
        return (
            await pool.query(`SELECT id FROM plays WHERE masterid = $1`, [
                masterId,
            ])
        ).rows.map((val) => {
            return val.id;
