...
class TokenRepository {
    ...
    async deleteFingerprint({ hash }: { hash: string }): Promise<boolean> {
        return ((await pool.query(`DELETE FROM refreshtokens WHERE fingerprint = $1;`, [hash])).rowCount as number) > 0;
    }
