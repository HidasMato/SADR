...
const SQLaddUsers = async () => {
    try {
        ...
        if (res.rows.length >= 1) await pool.query(`INSERT INTO admins (id, creategame, changegame, deletegame, createplay, changeplay, deleteplay, disactivplay, masterrights, mainadmin) VALUES ($1, True, True, True, True, True, True, True, True, True);`, [
            res.rows[0].id,
        ]);
